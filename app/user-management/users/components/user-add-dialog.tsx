...
const UserAddDialog = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      const message = t('users.addDialog.successMessage');
      toast.custom(
        () => (
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)}>
            <DialogBody className="pt-2.5 space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('users.addDialog.name')}</FormLabel>
                    <FormControl>
                      <Input placeholder={t('users.addDialog.namePlaceholder')} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('users.addDialog.email')}</FormLabel>
                    <FormControl>
                      <Input placeholder={t('users.addDialog.emailPlaceholder')} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('users.addDialog.role')}</FormLabel>
                    <FormControl>
                      <Select
