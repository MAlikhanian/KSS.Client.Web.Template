...
const UserList = () => {
  ...
  const fetchUsers = async ({
    ...
    if (!response.ok) {
      ...
      console.error('[UserList] API error:', {
        status: response.status,
        statusText: response.statusText,
        errorData,
      });
      throw new Error(errorData.message || t('messages.system_error'));
    }
    ...
    () => [
      {
        ...
        size: 300,
        meta: {
          headerTitle: t('users.table.columns.name'),
          skeleton: (
            <div className="flex items-center gap-3">
      ...
      },
      {
        ...
        meta: {
          headerTitle: t('users.table.columns.role'),
      ...
      },
      {
        ...
        size: 125,
        meta: {
          headerTitle: t('users.table.columns.status'),
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader
            title={t('users.table.columns.joined')}
            visibility={true}
            column={column}
          />
        ),
        ...
        size: 150,
        meta: {
          headerTitle: t('users.table.columns.joined'),
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader
            title={t('users.table.columns.lastSignIn')}
            visibility={true}
            column={column}
          />
        ),
        ...
        size: 175,
        meta: {
          headerTitle: t('users.table.columns.lastSignIn'),
          skeleton: <Skeleton className="w-20 h-7" />,
        },
        enableSorting: true,
        enableHiding: true,
      ...
      <CardHeader className="flex-col flex-wrap sm:flex-row items-stretch sm:items-center py-5">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
          <div className="relative">
            ...
            <Input
              placeholder={t('users.table.search.placeholder')}
              value={inputValue}
          ...
          >
            <SelectTrigger className="w-full sm:w-36">
              <SelectValue placeholder={t('users.table.filters.role.placeholder')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t('users.table.filters.role.allRoles')}</SelectItem>
          ...
          >
            <SelectTrigger className="w-full sm:w-36">
              <SelectValue placeholder={t('users.table.filters.status.placeholder')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t('users.table.filters.status.allUsers')}</SelectItem>
              {Object.entries(UserStatusProps).map(([status, { label }]) => {
                const statusKey = status.toLowerCase();
                const translatedLabel = t(`userManagement.users.table.status.${statusKey}`) || label;
                return (
