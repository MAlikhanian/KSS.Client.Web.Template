...
const UserProfileEditDialog = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      const message = t('users.editDialog.successMessage');
        ...
        <Form {...form}>
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('users.editDialog.name')}</FormLabel>
                  <FormControl>
                    <Input placeholder={t('users.editDialog.namePlaceholder')} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('users.editDialog.role')}</FormLabel>
                  <FormControl>
                    <Select
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('users.editDialog.status')}</FormLabel>
                  <FormControl>
                    <Select
                      onValueChange={(value) => field.onChange(value)}
                      defaultValue={field.value}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={t('users.editDialog.statusPlaceholder')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
                          {Object.entries(UserStatusProps).map(
                            ([status]) => {
                              const statusProps = getUserStatusProps(status as UserStatus, t);
                              return (
            ...
            <DialogFooter>
              ...
              >
                ...
                {t('users.editDialog.saveChanges')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
