      ...
      <Card>
        <CardContent>
          <dl className="grid grid-cols-[auto_1fr] gap-3 text-sm mb-5 [&_dt]:text-muted-foreground">
            <div className="grid grid-cols-subgrid col-span-2 items-baseline">
              <dt className="flex md:w-64">{t('users.profile.fullName')}</dt>
              <dd>{user.name || t('users.profile.notAvailable')}</dd>
            </div>
            <div className="grid grid-cols-subgrid col-span-2 items-baseline">
              <dt>{t('users.profile.emailAddress')}</dt>
              ...
              <dd>
                <span className="inline-flex items-center gap-1">
                  ...
                  {user.role?.isProtected && (
                    <Badge variant="outline">{t('users.profile.system')}</Badge>
                  )}
                </span>
              </dd>
            </div>
            <div className="grid grid-cols-subgrid col-span-2 items-baseline">
              <dt>{t('users.profile.status')}</dt>
              <dd>
            ...
            <div className="grid grid-cols-subgrid col-span-2 items-baseline">
              <dt>{t('users.profile.lastSignIn')}</dt>
              <dd>
                {user.lastSignInAt
                  ? formatDateTime(new Date(user.lastSignInAt))
                  : t('users.profile.never')}
              </dd>
            </div>
          </dl>
          <Button
          ...
          >
            {t('users.profile.editUserDetails')}
          </Button>
        </CardContent>
      </Card>
    );
