...
const getNotificationSettings = (t: (key: string) => string) => [
  {
    label: t('settings.notifications.types.stockAlerts.label'),
    description: t('settings.notifications.types.stockAlerts.description'),
    emailField: 'notifyStockEmail',
    webField: 'notifyStockWeb',
    roleIdsField: 'notifyStockRoleIds',
  },
  {
    label: t('settings.notifications.types.newOrders.label'),
    description: t('settings.notifications.types.newOrders.description'),
    emailField: 'notifyNewOrderEmail',
    webField: 'notifyNewOrderWeb',
    roleIdsField: 'notifyNewOrderRoleIds',
  },
  {
    label: t('settings.notifications.types.orderStatusUpdates.label'),
    description: t('settings.notifications.types.orderStatusUpdates.description'),
    emailField: 'notifyOrderStatusUpdateEmail',
    webField: 'notifyOrderStatusUpdateWeb',
    roleIdsField: 'notifyOrderStatusUpdateRoleIds',
  },
  {
    label: t('settings.notifications.types.paymentFailures.label'),
    description: t('settings.notifications.types.paymentFailures.description'),
    emailField: 'notifyPaymentFailureEmail',
    webField: 'notifyPaymentFailureWeb',
    roleIdsField: 'notifyPaymentFailureRoleIds',
  },
  {
    label: t('settings.notifications.types.systemErrors.label'),
    description: t('settings.notifications.types.systemErrors.description'),
      ...
      toast.custom(
        () => (
          <Alert variant="mono" icon="success">
            ...
            <AlertTitle>{t('settings.notifications.messages.settingsUpdated')}</AlertTitle>
          </Alert>
        ),
        {
          position: 'top-center',
        ...
        <Card>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <Popover>
                            ...
                            >
                              <Command>
                                <CommandInput placeholder={t('settings.notifications.searchRolesPlaceholder')} />
                                <CommandList>
                                  <CommandEmpty>{t('settings.notifications.noRolesFound')}</CommandEmpty>
                                  <CommandGroup>
                                    <ScrollArea>
                                      {roles?.map((role) => {
                                        const isSelected = (
                          ...
                          <div className="flex items-center flex-wrap gap-2">
                            {(
                              ...
                              <span className="text-muted-foreground">
                                {t('settings.notifications.notSet')}
                              </span>
                            )}
                          </div>
                        </div>
          ...
          <CardFooter className="flex justify-end gap-4 py-5 px-10">
            <Button type="button" variant="outline" onClick={handleReset}>
              {t('settings.notifications.buttons.reset')}
            </Button>
            <Button type="submit" disabled={isProcessing}>
              {isProcessing && <LoaderCircleIcon className="animate-spin" />}
              {t('settings.notifications.buttons.saveSettings')}
            </Button>
          </CardFooter>
        </Card>
      </Form>
