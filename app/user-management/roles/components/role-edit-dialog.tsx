  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      const message = isEdit
        ? t('roles.dialogs.edit.messages.roleUpdated')
        : t('roles.dialogs.edit.messages.roleAdded');
    ...
    <Dialog open={open} onOpenChange={closeDialog}>
      <DialogContent showCloseButton={false}>
        <DialogHeader>
          <DialogTitle>
            {role
              ? t('roles.dialogs.edit.title.edit')
              : t('roles.dialogs.edit.title.add')}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('roles.dialogs.edit.fields.roleName')}</FormLabel>
                  <FormControl>
                    <Input placeholder={t('roles.dialogs.edit.fields.roleNamePlaceholder')} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('roles.dialogs.edit.fields.slug')}</FormLabel>
                  <FormControl>
                    <Input
                      placeholder={t('roles.dialogs.edit.fields.slugPlaceholder')}
                      {...field}
                      disabled={!!role}
                    />
                  </FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('roles.dialogs.edit.fields.description')}</FormLabel>
                  <FormControl>
                    <Textarea placeholder={t('roles.dialogs.edit.fields.descriptionPlaceholder')} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={() => (
                <FormItem>
                  <FormLabel>{t('roles.dialogs.edit.fields.permissions')}</FormLabel>
                  <div className="flex items-center flex-wrap gap-1.5 text-2sm text-muted-foreground border border-input rounded-md px-3 py-3">
                    {selectedPermissions.length > 0 ? (
                      ...
                      <span className="text-sm text-muted-foreground">
                        {t('roles.dialogs.edit.fields.noPermissions')}
                      </span>
                    )}
                  </div>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" role="combobox">
                            {t('roles.dialogs.edit.fields.addPermissions')}
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent
                        ...
                        >
                          <Command>
                            <CommandInput placeholder={t('roles.dialogs.edit.fields.searchPermissions')} />
                            <CommandList>
                              <CommandEmpty>{t('roles.dialogs.edit.fields.noPermissionsFound')}</CommandEmpty>
                              <CommandGroup>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isProcessing}>
                ...
                {role
                  ? t('roles.dialogs.edit.buttons.updateRole')
                  : t('roles.dialogs.edit.buttons.createRole')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
