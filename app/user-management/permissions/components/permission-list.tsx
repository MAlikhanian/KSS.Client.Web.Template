  ...
  const columns = useMemo<ColumnDef<UserPermission>[]>(
    () => [
      ...
      },
      {
        ...
        accessorKey: 'name',
        header: ({ column }) => (
          <DataGridColumnHeader title={t('permissions.table.columns.permission')} column={column} />
        ),
        ...
        enableHiding: false,
        meta: {
          headerTitle: t('permissions.table.columns.permission'),
      ...
      },
      {
        ...
        accessorKey: 'slug',
        header: ({ column }) => (
          <DataGridColumnHeader title={t('permissions.table.columns.slug')} column={column} />
        ),
        cell: (info) => {
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('permissions.table.columns.description')} column={column} />
        ),
        cell: (info) => {
          const value = info.getValue() as string;
        ...
        enableHiding: false,
        meta: {
          headerTitle: t('permissions.table.columns.description'),
          skeleton: <Skeleton className="w-28 h-8" />,
        },
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('permissions.table.columns.createdAt')} column={column} />
        ),
        cell: (info) => {
          const value = info.getValue() as string;
        ...
        enableHiding: false,
        meta: {
          headerTitle: t('permissions.table.columns.createdAt'),
          skeleton: <Skeleton className="w-20 h-8" />,
        },
      },
      {
        id: 'actions',
        header: t('permissions.table.columns.actions'),
        cell: ({ row }) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent side="bottom" align="start">
              ...
              >
                {t('permissions.table.dropdown.editPermission')}
              </DropdownMenuItem>
              <DropdownMenuItem
                variant="destructive"
                onClick={() => {
              ...
              >
                {t('permissions.table.dropdown.deletePermission')}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ),
      ...
      <CardHeader className="flex-col flex-wrap sm:flex-row items-end items-stretch sm:items-center py-5">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
          ...
          >
            <SelectTrigger className="w-full sm:w-36">
              <SelectValue placeholder={t('permissions.table.filters.role.placeholder')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t('permissions.table.filters.role.allRoles')}</SelectItem>
        ...
        <CardToolbar>
          {deletePermissionIds.length > 0 && (
            ...
            >
              {t('permissions.table.actions.deletePermissions', { count: deletePermissionIds.length })}
            </Button>
          )}
          <Button
            disabled={isLoading && true}
          ...
          >
            <Plus />
            {t('permissions.table.actions.addPermission')}
          </Button>
        </CardToolbar>
      </CardHeader>
    );
