  ...
  const columns = useMemo<ColumnDef<IData>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountSecurityBackupAndRecovery.backup.columns.when')} column={column} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountSecurityBackupAndRecovery.backup.columns.details')} column={column} />
        ),
        cell: ({ row }) => (
          <div>
            <span className="leading-none font-medium text-sm text-mono">
            ...
            <span className="flex items-center gap-2 text-xs text-secondary-foreground font-normal">
              <span className="flex items-center gap-1">
                <File size={16} />
                <p>{t('accountSecurityBackupAndRecovery.backup.pages', { count: Number(row.original.details.pages) })}</p>
              </span>
              <span className="border-e border-e-input h-4"></span>
              <span className="flex items-center gap-1">
                <ImageIcon size={16} />
                <p>{t('accountSecurityBackupAndRecovery.backup.media', { count: Number(row.original.details.media) })}</p>
              </span>
            </span>
          </div>
        ),
      ...
      },
      {
        id: 'preview',
        header: () => '',
        cell: () => <Button variant="ghost">{t('accountSecurityBackupAndRecovery.backup.preview')}</Button>,
        enableSorting: false,
        size: 100,
        meta: {
          headerClassName: '',
      ...
      },
      {
        id: 'restore',
        header: () => '',
        cell: () => <Button variant="outline">{t('accountSecurityBackupAndRecovery.backup.restore')}</Button>,
        enableSorting: false,
        size: 100,
        meta: {
          headerClassName: '',
      ...
      <CardToolbar>
        <div className="flex flex-wrap items-center gap-2.5">
          <Label htmlFor="auto-update" className="text-sm">
            {t('accountSecurityBackupAndRecovery.backup.toolbar.cloudSync')}
          </Label>
        ...
        <DataGridColumnVisibility
          ...
          trigger={
            <Button variant="outline">
              <Settings2 />
              {t('accountSecurityBackupAndRecovery.backup.toolbar.columns')}
            </Button>
          }
        />
      </CardToolbar>
          ...
          <CardHeading>
            <div className="flex items-center gap-2.5">
              <div className="relative">
                ...
                <Input
                  placeholder={t('accountSecurityBackupAndRecovery.backup.searchPlaceholder')}
                  value={searchQuery}
