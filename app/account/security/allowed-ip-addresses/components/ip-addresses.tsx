  ...
  const columns = useMemo<ColumnDef<IData>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountSecurityAllowedIpAddresses.columns.status')} column={column} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountSecurityAllowedIpAddresses.columns.ipAddress')} column={column} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountSecurityAllowedIpAddresses.columns.lastSession')} column={column} />
        ),
        cell: ({ row }) => (
          <span className="text-secondary-foreground font-normal">
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountSecurityAllowedIpAddresses.columns.label')} column={column} />
        ),
        cell: ({ row }) => (
          <span className="text-secondary-foreground font-normal">
            {row.original.label}
        ...
        header: ({ column }) => (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex items-center gap-2">
                  ...
                  <DataGridColumnHeader
                    title={t('accountSecurityAllowedIpAddresses.columns.method')}
                    visibility={true}
                    column={column}
                  />
                </div>
              ...
              <TooltipContent>
                <p>{t('accountSecurityAllowedIpAddresses.columns.methodTooltip')}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ),
      ...
      <CardToolbar>
        <div className="flex items-center space-x-2">
          <Label htmlFor="auto-update" className="text-sm">
            {t('accountSecurityAllowedIpAddresses.toolbar.ipAllowlistEnabled')}
          </Label>
          <Switch size="sm" id="auto-update" defaultChecked />
        </div>
        <Button>{t('accountSecurityAllowedIpAddresses.toolbar.addIpAddress')}</Button>
        <DataGridColumnVisibility
          table={table}
          trigger={
            <Button variant="outline">
              <Settings2 />
              {t('accountSecurityAllowedIpAddresses.toolbar.columns')}
            </Button>
          }
        />
      </CardToolbar>
          ...
          <CardHeading>
            <div className="flex items-center gap-2.5">
              <div className="relative">
                ...
                <Input
                  placeholder={t('accountSecurityAllowedIpAddresses.searchPlaceholder')}
                  value={searchQuery}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline">
                    <Filter />
                    {t('accountSecurityAllowedIpAddresses.status')}
                ...
                <PopoverContent className="w-40 p-3" align="start">
                  <div className="space-y-3">
                    <div className="text-xs font-medium text-muted-foreground">
                      {t('accountSecurityAllowedIpAddresses.filters')}
                    </div>
                    <div className="space-y-3">
