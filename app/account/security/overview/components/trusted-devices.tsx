...
const TrustedDevices = () => {
  ...
  const items: ITrustedDevicesItems = [
    {
      ...
      datetime: `${t('accountSecurity.trustedDevices.active')}: Today at 9:03 AM`,
    },
    {
      logo: 'chrome.svg',
      browser: 'Chrome',
      ...
      datetime: `${t('accountSecurity.trustedDevices.active')}: Mar 18 at 9:03 AM`,
    },
  ];
  ...
  return (
    <Card className="min-w-full">
      <CardHeader>
        <CardTitle>{t('accountSecurity.trustedDevices.title')}</CardTitle>
        <DropdownMenu2
          trigger={
            <Button variant="ghost" mode="icon">
              <EllipsisVertical />
      ...
      <CardFooter className="justify-center">
        <Button mode="link" underlined="dashed" asChild>
          <Link href="#">{t('accountSecurity.trustedDevices.manageTrustedDevices')}</Link>
        </Button>
      </CardFooter>
    </Card>
  );
