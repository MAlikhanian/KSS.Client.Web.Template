...
const LoginSessions = () => {
          ...
          <div className="flex items-center grow gap-2.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-xs font-normal text-secondary-foreground">
                {t('accountSecurityOverview.loginSessions.connections', { count: row.connections })}
              </span>
            </div>
          </div>
        </TableCell>
  ...
  return (
    <Card className="min-w-full">
      <CardHeader>
        <CardTitle>{t('accountSecurityOverview.loginSessions.title')}</CardTitle>
        <DropdownMenu6
          trigger={
            <Button variant="ghost" mode="icon">
              <EllipsisVertical />
          ...
          <Table className="align-middle text-secondary-foreground text-sm">
            <TableHeader>
              <TableRow className="bg-accent/60">
                <TableHead className="text-start min-w-48 h-10">{t('accountSecurityOverview.loginSessions.tableHeaders.name')}</TableHead>
                <TableHead className="text-end min-w-20 h-10">
                  {t('accountSecurityOverview.loginSessions.tableHeaders.location')}
                </TableHead>
                <TableHead className="text-end min-w-20 h-10">
                  {t('accountSecurityOverview.loginSessions.tableHeaders.recentActivity')}
                </TableHead>
      ...
      <CardFooter className="justify-center">
        <Button mode="link" underlined="dashed" asChild>
          <Link href="#">{t('accountSecurityOverview.loginSessions.viewMore', { count: 64 })}</Link>
        </Button>
      </CardFooter>
    </Card>
  );
