        ...
        prev.map((file) => {
          ...
          if (newProgress > 50 && Math.random() < 0.1) {
            return {
              ...
              error: t('accountSettingsPlain.filesUpload.uploadFailed'),
            };
          }
  ...
  return (
    <Card className="pb-2.5">
      <CardHeader id="files_upload">
        <CardTitle>{t('accountSettingsPlain.filesUpload.title')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-5">
				...
				>
					...
					<div className="flex flex-col items-center gap-4">
						...
						<div className="space-y-2">
							<h3 className="text-lg font-semibold">{t('accountSettingsPlain.filesUpload.uploadYourFiles')}</h3>
							<p className="text-sm text-muted-foreground">{t('accountSettingsPlain.filesUpload.dragAndDrop')}</p>
							<p className="text-xs text-muted-foreground">
								{t('accountSettingsPlain.filesUpload.supportMultipleFiles', { size: formatBytes(maxSize) })}
							</p>
						</div>

						<Button onClick={openFileDialog}>
							<UploadIcon />
							{t('accountSettingsPlain.filesUpload.selectFiles')}
						</Button>
					</div>
				</div>
				...
				{uploadFiles.length > 0 && (
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-2">
							<h4 className="text-sm font-medium">{t('accountSettingsPlain.filesUpload.uploadProgress')}</h4>
							<div className="flex items-center gap-2">
								{completedCount > 0 && (
									<Badge size="sm" variant="success" appearance="light">
										{t('accountSettingsPlain.filesUpload.completed', { count: completedCount })}
									</Badge>
								)}
								{errorCount > 0 && (
									<Badge size="sm" variant="destructive" appearance="light">
										{t('accountSettingsPlain.filesUpload.failed', { count: errorCount })}
									</Badge>
								)}
								{uploadingCount > 0 && (
									<Badge size="sm" variant="secondary">
										{t('accountSettingsPlain.filesUpload.uploading', { count: uploadingCount })}
									</Badge>
								)}
							</div>
						</div>
						...
						<Button onClick={clearFiles} variant="outline" size="sm">
							{t('accountSettingsPlain.filesUpload.clearAll')}
						</Button>
					</div>
				)}
				...
				{errors.length > 0 && (
					<Alert variant="destructive" appearance="light">
						...
						<AlertContent>
							<AlertTitle>{t('accountSettingsPlain.filesUpload.uploadError')}</AlertTitle>
							<AlertDescription>
