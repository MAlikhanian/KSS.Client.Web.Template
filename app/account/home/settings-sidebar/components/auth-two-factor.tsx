  ...
  const items: IAuthTwoFactorItems = [
    {
      ...
      title: t('accountSettingsSidebar.authTwoFactor.textMessage'),
      description: t('accountSettingsSidebar.authTwoFactor.textMessageDescription'),
      checkbox: true,
    },
    {
      icon: ShieldCheck,
      title: t('accountSettingsSidebar.authTwoFactor.authenticatorApp'),
      description: t('accountSettingsSidebar.authTwoFactor.authenticatorAppDescription'),
      checkbox: false,
    },
  ];
  ...
  return (
    <Card>
      <CardHeader id="auth_two_factor">
        <CardTitle>{t('accountSettingsSidebar.authTwoFactor.title')}</CardTitle>
        <DropdownMenu6
          trigger={
      ...
      <CardContent>
        ...
        <div className="w-full">
          <div className="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-7">
            <Label className="flex w-full max-w-56">{t('accountSettingsSidebar.authTwoFactor.password')}</Label>
            <div className="flex flex-col tems-start grow gap-3 w-full">
              <Input
                type="text"
                placeholder={t('accountSettingsSidebar.authTwoFactor.enterPassword')}
                defaultValue={newPassword}
                onChange={(e) => setNewPassword(e.target.value)}
              />
              <span className="form-info text-mono font-normal">
                {t('accountSettingsSidebar.authTwoFactor.passwordDescription')}
              </span>
            </div>
          </div>
        </div>
        <div className="flex justify-end pt-2.5">
          <Button>{t('accountSettingsSidebar.authTwoFactor.setup')}</Button>
        </div>
      </CardContent>
    </Card>
  );
