...
const Connections = () => {
          ...
          <div className="flex items-center grow gap-2.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-xs font-normal text-secondary-foreground">
                {table.connections === 'none' ? t('accountSettingsEnterprise.connections.noConnections') : t('accountSettingsEnterprise.connections.connectionsCount', { count: table.connections })}
              </span>
            </div>
          </div>
        </TableCell>
        <TableCell className="py-2 text-end text-secondary-foreground font-medium">
          {table.jointLinks === 'none' ? t('accountSettingsEnterprise.connections.none') : table.jointLinks}
        </TableCell>
  ...
  return (
    <Card className="min-w-full">
      <CardHeader>
        <CardTitle>{t('accountSettingsEnterprise.connections.title')}</CardTitle>
        <DropdownMenu5
          trigger={
            ...
            <TableBody>
              <TableRow className="bg-accent/60">
                <TableCell className="text-start font-medium min-w-48 py-2.5">
                  {t('accountSettingsEnterprise.connections.tableHeaders.name')}
                </TableCell>
                <TableCell className="text-end font-medium min-w-28 py-2.5">
                  {t('accountSettingsEnterprise.connections.tableHeaders.jointLinks')}
                </TableCell>
                <TableCell className="text-end font-medium min-w-20 py-2.5">
                  {t('accountSettingsEnterprise.connections.tableHeaders.status')}
                </TableCell>
      ...
      <CardFooter className="justify-center">
        <Button mode="link" underlined="dashed" asChild>
          <Link href="#">{t('accountSettingsEnterprise.connections.viewMore')}</Link>
        </Button>
      </CardFooter>
    </Card>
  );
