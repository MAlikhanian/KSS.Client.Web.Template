  ...
  return (
    <Card>
      <CardHeader className="gap-2" id="settings_set_goal">
        <CardTitle>{t('accountSettingsEnterprise.companyDocuments.title')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-5">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium">
                {t('accountSettingsEnterprise.companyDocuments.dropFilesHere')}{' '}
                <button
                  type="button"
                  onClick={openFileDialog}
                  className="cursor-pointer text-primary underline-offset-4 hover:underline"
                >
                  {t('accountSettingsEnterprise.companyDocuments.browseFiles')}
                </button>
              </p>
              <p className="text-xs text-muted-foreground">
                {t('accountSettingsEnterprise.companyDocuments.maxFileSize', { size: formatBytes(maxSize), count: maxFiles })}
              </p>
            </div>
          </div>
        </div>
        ...
        {uploadFiles.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-medium">{t('accountSettingsEnterprise.companyDocuments.files', { count: uploadFiles.length })}</h3>
              <div className="flex gap-2">
                <Button onClick={openFileDialog} variant="outline" size="sm">
                  <CloudUpload />
                  {t('accountSettingsEnterprise.companyDocuments.addFiles')}
                </Button>
                <Button onClick={clearFiles} variant="outline" size="sm">
                  <Trash2 />
                  {t('accountSettingsEnterprise.companyDocuments.removeAll')}
                </Button>
              </div>
            </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="text-xs">
                      <TableHead className="h-9">{t('accountSettingsEnterprise.companyDocuments.tableHeaders.name')}</TableHead>
                      <TableHead className="h-9">{t('accountSettingsEnterprise.companyDocuments.tableHeaders.type')}</TableHead>
                      <TableHead className="h-9">{t('accountSettingsEnterprise.companyDocuments.tableHeaders.size')}</TableHead>
                      <TableHead className="h-9 w-[100px] text-end">{t('accountSettingsEnterprise.companyDocuments.tableHeaders.actions')}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                            ...
                            <p className="flex items-center gap-1 truncate text-sm font-medium">
                              ...
                              {fileItem.status === 'error' && (
                                <Badge variant="destructive" size="sm" appearance="light">
                                  {t('accountSettingsEnterprise.companyDocuments.error')}
                                </Badge>
                              )}
                            </p>
                          </div>
        ...
        {errors.length > 0 && (
          <Alert variant="destructive" appearance="light">
            ...
            <AlertContent>
              <AlertTitle>{t('accountSettingsEnterprise.companyDocuments.uploadError')}</AlertTitle>
              <AlertDescription>
