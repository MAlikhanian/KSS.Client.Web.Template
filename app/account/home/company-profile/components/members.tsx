...
const Members = ({ url }: IMembersProps) => {
    ...
    return (
      <TableRow key={index}>
          ...
          <div className="flex items-center grow gap-2.5">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs font-normal text-secondary-foreground">
                {table.connections} {t('accountCompanyProfile.members.connections')}
              </span>
            </div>
          </div>
        </TableCell>
        ...
        <TableCell className="text-end">
          ...
          >
            {table.disabled ? t('accountCompanyProfile.members.disabled') : t('accountCompanyProfile.members.enabled')}
          </Badge>
        </TableCell>
  ...
  return (
    <Card className="min-w-full">
      <CardHeader>
        <CardTitle>{t('accountCompanyProfile.members.title')}</CardTitle>
        <div className="flex items-center gap-5">
          <div className="flex items-center gap-2">
            <Label htmlFor="auto-update" className="text-sm">
              {t('accountCompanyProfile.members.enforce2FA')}
            </Label>
      ...
      <CardFooter className="justify-center">
        <Button mode="link" underlined="dashed" asChild>
          <Link href={url}>{t('accountCompanyProfile.members.viewMore')}</Link>
        </Button>
      </CardFooter>
    </Card>
  );
