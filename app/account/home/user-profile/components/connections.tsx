...
const Connections = ({ url }: IConnectionsProps) => {
        ...
        <div className="flex items-center grow gap-2.5">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs font-normal text-secondary-foreground leading-3">
              {item.connections} {t('accountUserProfile.calendarAccounts.connections')}
            </span>
          </div>
        </div>
      </TableCell>
  ...
  return (
    <Card className="min-w-full">
      <CardHeader>
        <CardTitle>{t('accountUserProfile.connections.title')}</CardTitle>
        <DropdownMenu4
          trigger={
            <Button variant="ghost" mode="icon">
              <EllipsisVertical />
            ...
            <TableBody>
              <TableRow className="bg-accent/60">
                <TableCell className="text-start font-normal min-w-48 py-2.5">
                  {t('accountUserProfile.connections.name')}
                </TableCell>
                <TableCell className="text-end font-medium min-w-20 py-2.5">
                  {t('accountUserProfile.connections.jointLinks')}
                </TableCell>
                <TableCell className="text-end font-medium min-w-20 py-2.5">
                  {t('accountUserProfile.connections.status')}
                </TableCell>
      ...
      <CardFooter className="justify-center">
        <Button mode="link" underlined="dashed" asChild>
          <Link href={url}>{t('accountUserProfile.connections.viewMore')}</Link>
        </Button>
      </CardFooter>
    </Card>
  );
