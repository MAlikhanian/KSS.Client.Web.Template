...
function ActionsCell({ row }: { row: Row<IData> }) {
  ...
  const handleCopyId = () => {
    ...
    const message = t('accountInviteAFriend.invites.inviteIdCopied', { id: row.original.id });
    toast.custom(
      (t) => (
        <Alert
          variant="mono"
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent side="bottom" align="end">
        <DropdownMenuItem onClick={() => {}}>{t('accountInviteAFriend.invites.actions.edit')}</DropdownMenuItem>
        <DropdownMenuItem onClick={handleCopyId}>{t('accountInviteAFriend.invites.actions.copyId')}</DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem variant="destructive" onClick={() => {}}>
          {t('accountInviteAFriend.invites.actions.delete')}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
  ...
  const columns = useMemo<ColumnDef<IData>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountInviteAFriend.invites.columns.member')} column={column} />
        ),
        cell: ({ row }) => (
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-sm text-secondary-foreground font-normal">
                {row.original.member.tasks} {t('accountInviteAFriend.invites.tasks')}
              </span>
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountInviteAFriend.invites.columns.location')} column={column} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountInviteAFriend.invites.columns.status')} column={column} />
        ),
        cell: ({ row }) => {
          const statusLabel = row.original.status.label;
          const translatedLabel = statusLabel === 'Accepted' 
            ? t('accountInviteAFriend.invites.statusLabels.accepted')
            : statusLabel === 'Pending'
            ? t('accountInviteAFriend.invites.statusLabels.pending')
            : statusLabel === 'Declined'
            ? t('accountInviteAFriend.invites.statusLabels.declined')
            : statusLabel;
          return (
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountInviteAFriend.invites.columns.recentActivity')} column={column} />
        ),
        cell: ({ row }) => (
    ...
    return (
      <CardToolbar>
        <Label htmlFor="auto-update" className="text-sm">
          {t('accountInviteAFriend.invites.toolbar.activeUsers')}
        </Label>
        ...
        <DataGridColumnVisibility
          ...
          trigger={
            <Button variant="outline">
              <Settings2 />
              {t('accountInviteAFriend.invites.toolbar.columns')}
            </Button>
          }
        />
      </CardToolbar>
          ...
          <CardHeading>
            <div className="flex items-center gap-2.5">
              <div className="relative">
                ...
                <Input
                  placeholder={t('accountInviteAFriend.invites.searchPlaceholder')}
                  value={searchQuery}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline">
                    <Filter />
                    {t('accountInviteAFriend.invites.status')}
                ...
                <PopoverContent className="w-40 p-3" align="start">
                  <div className="space-y-3">
                    <div className="text-xs font-medium text-muted-foreground">
                      {t('accountInviteAFriend.invites.filters')}
                    </div>
                    <div className="space-y-3">
