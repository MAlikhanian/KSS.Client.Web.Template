...
const Invoicing = () => {
  ...
  const columns = useMemo<ColumnDef<IData>[]>(
    () => {
      const ColumnInputFilter = <TData, TValue>({
        ...
        return (
          <Input
            placeholder={t('accountBillingHistory.filter')}
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader
            title={t('accountBillingHistory.columnHeaders.member')}
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountBillingHistory.columnHeaders.status')} column={column} />
        ),
        cell: (info) => {
          const variant = info.row.original
            .status as keyof BadgeProps['variant'];
          const statusKey = info.row.original.label.toLowerCase().replace(/\s+/g, '');
          const statusLabel = t(`accountBillingHistory.status.${statusKey}`);
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountBillingHistory.columnHeaders.date')} column={column} />
        ),
        cell: (info) => {
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountBillingHistory.columnHeaders.dueDate')} column={column} />
        ),
        cell: (info) => {
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title={t('accountBillingHistory.columnHeaders.amount')} column={column} />
        ),
        cell: (info) => {
        ...
        cell: () => {
          return (
            <Button mode="link" underlined="dashed">
              {t('accountBillingHistory.download')}
            </Button>
          );
        },
        size: 90,
    ...
    return (
      <CardToolbar>
        <Button variant="outline">
          <Download />
          {t('accountBillingHistory.downloadPdf')}
        </Button>
        <DataGridColumnVisibility
          ...
          trigger={
            <Button variant="outline">
              <Settings2 />
              {t('accountBillingHistory.columns')}
            </Button>
          }
        />
      </CardToolbar>
  ...
  return (
    ...
    >
      <Card>
        <CardHeader>
          <CardTitle>{t('accountBillingHistory.title')}</CardTitle>
          <Toolbar />
        </CardHeader>
        <CardTable>
          <ScrollArea>
