  ...
  return (
    <div className="flex gap-5 lg:gap-7.5">
      {years.map((year, index) => (
        ...
        >
          <CardHeader>
            <CardTitle>{t('accountActivity.activity.title')}</CardTitle>
            <div className="flex items-center space-x-2.5">
              <Label htmlFor="simple-switch" className="text-sm">
                {t('accountActivity.activity.autoRefresh')}:
              </Label>
              {isSwitchOn ? t('accountActivity.activity.on') : t('accountActivity.activity.off')}
              <Switch
                id="simple-switch"
                size="sm"
                className="ms-2"
          ...
          <CardFooter className="justify-center">
            <Button mode="link" underlined="dashed" asChild>
              <Link href="#">{t('accountActivity.activity.allTimeActivity')}</Link>
            </Button>
          </CardFooter>
        </Card>
      ))}
