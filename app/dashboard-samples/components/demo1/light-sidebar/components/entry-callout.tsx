        ...
        <CardContent className="p-10 bg-[length:80%] rtl:[background-position:-70%_25%] [background-position:175%_25%] bg-no-repeat entry-callout-bg">
          <div className="flex flex-col justify-center gap-4">
            ...
            <h2 className="text-xl font-semibold text-mono">
              {t('dashboard.entryCallout.connectToday')} <br />
              {t('dashboard.entryCallout.keenThemesNetwork')}{' '}
              <Button mode="link" asChild className="text-xl font-semibold">
                <Link href="#">{t('dashboard.entryCallout.keenThemesNetwork')}</Link>
              </Button>
            </h2>
            <p className="text-sm font-normal text-secondary-foreground leading-5.5">
              {t('dashboard.entryCallout.description')}
            </p>
          </div>
        </CardContent>
        <CardFooter className="justify-center">
          <Button mode="link" underlined="dashed" asChild>
            <Link href="#">{t('dashboard.entryCallout.getStarted')}</Link>
          </Button>
        </CardFooter>
      </Card>
    </Fragment>
